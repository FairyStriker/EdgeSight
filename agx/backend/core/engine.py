...
class DeepStreamPipeline:
    ...
    def _create_pipeline(self):
        ...
        # AGX: NVJPG 하드웨어 인코더 사용 (nvjpegenc). 미사용 가능 시 jpegenc 로 fallback.
        # nvjpegenc 는 NVMM I420 입력을 요구하므로 capsfilter 한 단을 둔다.
        ...
        if jpegenc is not None:
            ...
            logger.info("JPEG 인코더: nvjpegenc (NVJPG 하드웨어)")
        else:
            logger.warning("nvjpegenc 사용 불가 — jpegenc(소프트웨어)로 fallback")
        ...
        if jpeg_caps is not None:
            # nvvidconv2 → capsfilter(NVMM I420) → nvjpegenc(HW) → appsink
            nvvidconv2.link(jpeg_caps)
            jpeg_caps.link(jpegenc)
        else:
            # 소프트웨어 fallback (system memory raw → jpegenc)

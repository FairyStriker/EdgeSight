...
def _gst_nvenc_normalize(src_path: str, dst_path: str) -> None:
    """AGX NVENC 하드웨어 인코더로 영상 정규화.

    GStreamer 파이프라인:
        uridecodebin (NVDEC 자동) → nvvideoconvert → NVMM/NV12
        → nvv4l2h264enc (NVENC, main profile, GOP 30) → h264parse → qtmux → filesink
    """
...
def _ffmpeg_normalize(src_path: str, dst_path: str) -> None:
    """ffmpeg libx264 정규화 (소프트웨어, fallback 용).
...
def _normalize_video(src_path: str, dst_path: str) -> str:
    """AGX: NVENC 우선, 실패 시 ffmpeg/libx264 fallback. 사용된 인코더 이름 반환."""
    try:
        _gst_nvenc_normalize(src_path, dst_path)
        return "NVENC (nvv4l2h264enc)"
    except Exception as e:
        logger.warning("NVENC 정규화 실패 — ffmpeg/libx264 fallback. 사유: %s", e)
        # NVENC 시도에서 부분 결과 파일이 남았을 수 있으므로 정리

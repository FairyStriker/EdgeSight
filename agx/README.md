...
NVIDIA **Jetson AGX Orin** 용 엣지 AI 영상 관제 시스템.
DeepStream(GStreamer) 기반 실시간 객체 탐지/트래킹과 React 기반 관리자 UI를 제공한다.

> 🚧 **상태**: Jetson Orin Nano 검증본을 베이스로 시작. AGX 특화 적용 진행 중.

## AGX 전용 변경 (nano 대비)

| 영역 | nano | AGX (이 폴더) |
|---|---|---|
| MJPEG 스트림 JPEG 인코딩 | `jpegenc` (CPU) | **`nvjpegenc` (NVJPG 하드웨어)** ※ 실패 시 jpegenc 자동 fallback |
| 영상 파일 정규화 H.264 인코딩 | `ffmpeg libx264` (CPU) | **GStreamer `nvv4l2h264enc` (NVENC 하드웨어)** ※ 실패 시 ffmpeg libx264 자동 fallback |

→ AGX Orin의 NVENC / NVJPG 하드웨어 인코더를 활용해 CPU 부담을 크게 낮춤.
→ 어떤 인코더가 실제로 사용됐는지는 `/api/jobs` 응답의 `message` 또는 백엔드 로그에서 확인 가능.

> nano와 동일하게 `frontend/`, 모델/설정 관리, DeepStream 파이프라인 구조는 그대로 유지.
